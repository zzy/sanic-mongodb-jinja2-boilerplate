...
class RegisterForm(forms.Form):
    ...
    captcha = forms.CharField(label = _('验证码'), widget = CaptchaWidget(), help_text = ' * 请输入图片上的字符。')
    username = forms.CharField(label = _('用户帐号'), max_length = 30, help_text = ' * 任何字符均可')
    ...
    photo = forms.ImageField(label = _('个人形象'), required = False, help_text = '须小于30KB，否则忽略')
    email = forms.EmailField(label = _('电子邮箱'), max_length = 75, help_text = ' * 激活帐号所必需')
    #is_email_public = forms.BooleanField(label = _(u'邮箱公开'), required = False, help_text = u' * 是否向其它玩家公开邮箱')
    password = forms.CharField(label = _('密钥'), widget = forms.PasswordInput, help_text = ' * 五个字符或以上')
    password_again = forms.CharField(label = _('密钥确认'), widget = forms.PasswordInput)
    ...
    tos = forms.BooleanField(label=_('理解并且同意注册条款'), widget=forms.CheckboxInput)
    ...
    def clean_username(self):
        if len(self.cleaned_data['username']) < 2:
            raise forms.ValidationError(_('用户帐号最少需要2个字符。'))
        try:
            User.objects.get(username__iexact = self.cleaned_data['username'])
        except User.DoesNotExist:
            return self.cleaned_data['username']
        raise forms.ValidationError(_('此用户帐号已经被使用。'))

    def clean_email(self):
        if User.objects.filter(email__iexact = self.cleaned_data['email']):
            raise forms.ValidationError(_('邮件地址已被使用。'))
        return self.cleaned_data['email']

    def clean(self):
        if 'password' in self.cleaned_data and 'password_again' in self.cleaned_data:
            if len(self.cleaned_data['password']) < 5:
                raise forms.ValidationError(_('密码最少需要5位。'))
            if self.cleaned_data['password'] != self.cleaned_data['password_again']:
                raise forms.ValidationError(_('两次输入的密码必须相同。'))
...
class RegisterNoEmail(RegisterForm):
    ...
    def clean_email(self):
        ...
        if email_domain in self.bad_domains:
            raise forms.ValidationError(_('本站拒绝此域邮箱。'))
        return self.cleaned_data['email']
...
class MessageForm(forms.Form):
    ...
    receiver = forms.CharField(label = _('接收人'), max_length = 120, help_text = '多人接收以\',\'号分隔')
    title = forms.CharField(label = _('标题'), max_length = 50, help_text = '勿超过50字符')
    message = forms.CharField(label = _('消息'), widget = forms.Textarea)
    sent_time = forms.CharField(label = _('发送时间'), max_length = 20)
